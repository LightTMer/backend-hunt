...
from prisma.partials import ThreadPartial, ForumPartial
...
async def lifespan(app: FastAPI):
    await db.connect()
    yield  # после yield выполняется код при остановке приложения, все что до yield, выполнятеся при запуске приложения
    await db.disconnect()
